...
GameScreenLevel1::GameScreenLevel1(SDL_Renderer* renderer, GameScreenManager* gameScreen) : GameScreen(renderer)
{
	SetUpLevel();
...
GameScreenLevel1::~GameScreenLevel1()
{
	...
	delete m_bounceOrb[1];
	m_bounceOrb[1] = nullptr;

	delete m_bounceOrb[2];
	m_bounceOrb[2] = nullptr;
...
bool GameScreenLevel1::SetUpLevel()
{
	...
	m_floor[0] = new GameObject(m_renderer, "Images/floor.png", Vector2D(16, 448));
	m_floor[1] = new GameObject(m_renderer, "Images/floor.png", Vector2D(1008, 448));
	m_rightWall[0] = new GameObject(m_renderer, "Images/wall.png", Vector2D(240, 480));
	m_leftWall[0] = new GameObject(m_renderer, "Images/wall.png", Vector2D(1008, 480));
	...
	m_character = new Character(m_renderer, "Images/player.png", Vector2D(32, 316));

	//sets up orb objects
	m_bounceOrb[0] = new GameObject(m_renderer, "Images/bounceOrb.png", Vector2D(400, 352));
	m_bounceOrb[1] = new GameObject(m_renderer, "Images/bounceOrb.png", Vector2D(750, 352));
	m_bounceOrb[2] = new GameObject(m_renderer, "Images/bounceOrb.png", Vector2D(575, 352));
	...
	m_exitDoor = new GameObject(m_renderer, "Images/exitDoor.png", Vector2D(1088, 320));
...
void GameScreenLevel1::Render()
{
	...
	m_gameText[1]->SetColor(SDL_Color{ 255, 238, 0, 0 });
	m_gameText[2]->SetColor(SDL_Color{ 0, 239, 225, 0 });
	m_gameText[3]->SetColor(SDL_Color{ 255, 238, 0, 0 });
	m_gameText[4]->SetColor(SDL_Color{ 255, 238, 0, 0 });

	//controls what the text says and its world position
	m_gameText[0]->RenderAt("LEVEL 1", 20, 20);
	m_gameText[1]->RenderAt("Hit the ", 300, 75);
	m_gameText[2]->RenderAt("BLUE ", 455, 75);
	m_gameText[3]->RenderAt("orbs to bounce,", 600, 75);
	m_gameText[4]->RenderAt("the direction you hit them decides where you bounce", 55, 125);
	...
	m_bounceOrb[1]->Render();
	m_bounceOrb[2]->Render();
...
#pragma region Orb Collisions
	...
	if (Collisions::Instance()->Box(m_meleeAttacks[0]->GetCollisionBox(), m_bounceOrb[1]->GetCollisionBox()))
	{
		m_character->Bounce(deltaTime, "Left", 1200);
	}
	if (Collisions::Instance()->Box(m_meleeAttacks[0]->GetCollisionBox(), m_bounceOrb[2]->GetCollisionBox()))
	{
		m_character->Bounce(deltaTime, "Left", 1200);
	}
	...
	if (Collisions::Instance()->Box(m_meleeAttacks[1]->GetCollisionBox(), m_bounceOrb[1]->GetCollisionBox()))
	{
		m_character->Bounce(deltaTime, "Right", 1200);
	}
	if (Collisions::Instance()->Box(m_meleeAttacks[1]->GetCollisionBox(), m_bounceOrb[2]->GetCollisionBox()))
	{
		m_character->Bounce(deltaTime, "Right", 1200);
	}
	...
	if (Collisions::Instance()->Box(m_meleeAttacks[2]->GetCollisionBox(), m_bounceOrb[1]->GetCollisionBox()))
	{
		m_character->Bounce(deltaTime, "Down", 1200);
	}
	if (Collisions::Instance()->Box(m_meleeAttacks[2]->GetCollisionBox(), m_bounceOrb[2]->GetCollisionBox()))
	{
		m_character->Bounce(deltaTime, "Down", 1200);
	}
	...
	if (Collisions::Instance()->Box(m_meleeAttacks[3]->GetCollisionBox(), m_bounceOrb[1]->GetCollisionBox()))
	{
		m_character->Bounce(deltaTime, "Up", 1200);
	}
	if (Collisions::Instance()->Box(m_meleeAttacks[3]->GetCollisionBox(), m_bounceOrb[2]->GetCollisionBox()))
	{
		m_character->Bounce(deltaTime, "Up", 1200);
	}
...
void GameScreenLevel1::Inputs(SDL_Event e)
{
	//Keyboard inputs
	switch (e.type)
	{
